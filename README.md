...
Works, because the data is built at deploy time rather than fetched per
request. `npm run build` (which Vercel runs automatically) executes
`scripts/build-snapshot.js`, fetching all five boroughs' AADT stations and the
NYC counts aggregation into `data/dataset-snapshot.json` (~2.6 MB, ~100s).
At runtime `datasetCache` seeds itself from that file, so the default
all-boroughs view renders in well under a second with no upstream calls.

**Two files must be present in the deployment**, both under `data/` and pulled
into the function bundle by `includeFiles` in `vercel.json`:

- `geocode-cache.json` — **committed to the repo.** Not a disposable cache:
  regenerating it is ~5,400 Nominatim lookups at 1 req/sec (~1.6 hours).
  Without it no AADT station has coordinates, so no spatial match is possible,
  every segment lacks a recent estimate, and the list filter drops all of them
  — the app returns an empty table with no error.
- `dataset-snapshot.json` — regenerated by every build, and committed as a
  fallback in case a build-time fetch fails.

`data/dataset-cache.json` stays gitignored: it is genuine transient runtime
state, and a committed copy would serve stale counts while masking a broken
fetch.

Remaining serverless caveat: instances share no memory, so the loading bar
(which polls a *different invocation* than the one doing the work) will
usually report nothing. With the snapshot in place loads are fast enough that
this rarely matters.

### Persistent host vs serverless

Both are supported. The persistent host is still the better fit — it can
refresh data on its own TTL, run the background geocoder to extend the
geocode cache, and show real load progress. Serverless is fixed to whatever
the last build captured, so a data refresh means a redeploy.
